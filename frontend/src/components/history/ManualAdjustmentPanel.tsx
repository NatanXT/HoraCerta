...
export function ManualAdjustmentPanel({ day, onClose, onSaved }: ManualAdjustmentPanelProps) {
  ...
  return (
    <div className="bg-slate-900 border border-slate-700/80 rounded-2xl p-6 shadow-2xl space-y-6 mt-4 transition-all">
      {/* Panel Header */}
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between pb-4 border-b border-slate-800 gap-3">
        ...
        <button
          type="button"
          onClick={onClose}
          className="self-end sm:self-auto text-slate-400 hover:text-white p-1.5 rounded-lg hover:bg-slate-800 transition-colors"
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            {intervals.map((interval, index) => (
              <div
                key={index}
                className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 p-3 bg-slate-950/50 rounded-xl border border-slate-800"
              >
                <div className="flex-1 flex items-center gap-2">
                  ...
                  <div className="flex-1 flex items-center gap-2">
                    <div className="flex-1">
                      <label htmlFor={`clockIn-${index}`} className="sr-only">
                        Entrada Intervalo {index + 1}
                      </label>
                      <input
                        id={`clockIn-${index}`}
                        type="time"
                        value={interval.clockIn}
                        onChange={(e) => handleIntervalChange(index, 'clockIn', e.target.value)}
                        required
                        className="w-full bg-slate-900 border border-slate-700/80 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 font-mono"
                      />
                    </div>
                    <span className="text-slate-500 font-mono text-sm">–</span>
                    <div className="flex-1">
                      <label htmlFor={`clockOut-${index}`} className="sr-only">
                        Saída Intervalo {index + 1}
                      </label>
                      <input
                        id={`clockOut-${index}`}
                        type="time"
                        value={interval.clockOut}
                        onChange={(e) => handleIntervalChange(index, 'clockOut', e.target.value)}
                        required
                        className="w-full bg-slate-900 border border-slate-700/80 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 font-mono"
                      />
                    </div>
                ...
                {intervals.length > 1 && (
                  <button
                    ...
                    className="self-end sm:self-center p-2 text-slate-400 hover:text-rose-400 hover:bg-rose-500/10 rounded-lg transition-colors"
        ...
        <div className="flex items-center justify-end gap-3 pt-2">
          <button
            ...
            onClick={onClose}
            disabled={saving}
            className="inline-flex items-center gap-1.5 px-4 py-2.5 rounded-xl border border-slate-700/80 text-xs font-semibold text-slate-300 hover:bg-slate-800 hover:text-white transition-colors disabled:opacity-50"
          >
          ...
          <button
            type="submit"
            disabled={saving}
            className="inline-flex items-center gap-1.5 px-5 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-xs font-semibold text-white shadow-lg shadow-indigo-600/20 transition-all disabled:opacity-50"
          >
      ...
      <div className="pt-6 border-t border-slate-800 space-y-4">
        ...
        {!loadingAdjustments && adjustments.length === 0 && (
          <p className="text-xs text-slate-500 italic py-2">
            Nenhum ajuste manual realizado neste dia.
          </p>
        )}
